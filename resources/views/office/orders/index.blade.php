...
<div class="card">
    ...
    <div class="card-body pt-0">
        <table id="kt_datatable_example_2" class="table table-striped table-row-bordered gy-5 gs-7">
            ...
            <tbody>
                @foreach(auth()->user()->Office->Orders as $order)
                <tr>
                    ...
                    <td>{{$start_date[0]}}</td>
                    <td>
                        <a href="{{route('office.order_show',encrypt($order->id))}}" class="btn btn-icon btn-sm btn-primary">
						<i class="bi bi-eye-fill fs-1x"></i>
                        </a>
						<a target="_blank" href="{{route('office.order_invoice',$order->id)}}" class="btn btn-icon btn-sm btn-primary">
